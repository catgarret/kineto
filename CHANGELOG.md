...
- Make deterministic CI checks fail fast instead of retrying the same lint/Node contract/package error three times. Network-backed consumer/framework installs and browser QA keep their retry paths; lint, core contract/package loops and the final dry-run pack now surface the first real failure immediately. Chromium provisioning is deferred until the deterministic contract/package gates pass, but still occurs before consumer/integration QA because that lane launches Chromium itself. Source/package failures therefore avoid browser/OS provisioning without starving integration tests of their browser dependency.
...
- 결정적인 CI 검사는 같은 lint/Node contract/package 오류를 세 번 반복하지 않고 첫 실패에서 바로 종료하도록 바꿨습니다. 네트워크가 필요한 consumer/framework 설치와 브라우저 QA의 retry는 유지하고, lint·핵심 contract/package 루프·최종 dry-run pack은 실제 첫 오류를 즉시 보여줍니다. Chromium 설치도 결정적인 contract/package gate 뒤로 미루되 Chromium을 직접 띄우는 consumer/integration QA보다는 앞에 둡니다. 소스/패키지 오류에서는 브라우저·OS 의존성 설치를 피하면서 integration 테스트의 실제 브라우저 의존성은 유지합니다.
