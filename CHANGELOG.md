...
- Make CI failures readable without a GitHub login. Test steps load `tests/ci-annotate.mjs`, which turns a failing test process into one public annotation naming the script, the error and its last output lines (escaped so no text can start another workflow command); job logs need a signed-in account, annotations do not. `release:ship` now waits up to 75 minutes for CI, polling every 90 seconds to stay under the anonymous API limit.
...
- GitHub 로그인 없이도 CI 실패 원인을 읽을 수 있게 했습니다. 테스트 단계가 `tests/ci-annotate.mjs`를 불러와, 실패한 테스트 프로세스마다 스크립트 이름·에러·마지막 출력 몇 줄을 공개 annotation 하나로 남깁니다(다른 워크플로 명령을 만들 수 없게 이스케이프). job 로그는 로그인이 필요하지만 annotation은 필요 없습니다. `release:ship`의 CI 대기는 최대 75분, 익명 API 한도를 넘지 않게 90초 간격으로 확인합니다.
