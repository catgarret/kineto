...
- `release:ship`은 `main`을 푸시한 뒤 그 커밋의 CI가 통과할 때까지 기다렸다가 태그를 만듭니다. 전에는 `main`과 태그를 함께 푸시해서 `v0.12.0`이 CI가 실패한 커밋에 태그되어 버전 번호만 소진됐습니다. 대기는 토큰 없이 공개 GitHub Actions API를 읽습니다(`scripts/ci-status.mjs`). CI가 실패하면 태그가 생기지 않으므로 같은 명령을 다시 실행하면 됩니다. `--skip-ci-wait`로 건너뛸 수 있습니다. 게이트: `tests/release-automation.mjs`.
