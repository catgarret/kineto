...
- Synchronize the Cover Reveal mask regression on its actual clip-path state instead of a fixed delay, preserving the WebKit assertion while tolerating slow first-paint delivery.
...
- Cover Reveal mask 회귀 검사를 고정 지연이 아닌 실제 clip-path 상태에 동기화해 WebKit의 느린 첫 페인트에서도 검증을 약화하지 않고 통과하도록 합니다.
