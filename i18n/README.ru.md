...
Kineto — библиотека из 52 интерактивных модулей (движение, медиа, скролл, лоадер, текст), которые подключаются одним атрибутом `data-kt-*` или точно управляются через JavaScript API. Ядро не имеет обязательных зависимостей; в неподдерживаемых браузерах и на слабых устройствах эффекты отключаются, а контент остаётся нетронутым.
...
Стандартные CDN-движки используют проверку целостности SHA-384. При изменении URL также задайте integrity для соответствующего файла.
...
| `lazy` | `data-kt-lazy` | Скелет·Пиксель·Печать·Растворение·Дизеринг·ASCII·Полутон загрузки (изображение·видео) |
...
Обновление параметров в адаптерах подчиняется явной политике замены. React
пересоздаёт модуль только при изменении `type` или значения в `dependencies`;
`useKineto()` во Vue делает то же самое для `watchSources`, а `v-motion` заменяет
экземпляр при изменении привязки. Vue принимает объект options, ref или getter;
если при замене нужно прочитать значения, изменившиеся после `setup()`, используйте
ref/getter. Адаптер вычисляет источник параметров непосредственно перед каждым
вызовом create. Перед заменой всегда выполняется очистка. Адаптеры не пытаются
угадать, поддерживает ли конкретный модуль частичный вызов `updateModule()`. Для
императивных обновлений в реальном времени сохраните возвращённый экземпляр или
вызывайте Core API напрямую.
