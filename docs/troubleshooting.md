...
## 뒤로가기를 눌렀더니 페이지 내용이 사라짐 (Page Transition)

`data-kt-page-transition`이 붙은 페이지에서 페이지 안 앵커(`#section`)로 이동한 뒤 **뒤로가기**를
누르면 컨테이너 안이 비어 버리는 문제가 v0.11.0까지 있었습니다. 해시만 다른 history 항목은
같은 문서 안의 이동이라 브라우저가 이미 할 일을 다 했는데, 모듈이 그때도 페이지를 다시
가져와 컨테이너를 갈아 끼웠기 때문입니다. 그러면 **로드 이후에 페이지가 만든 것**이 전부
사라집니다 — 스크립트가 생성한 섹션, 마운트된 패널, 프레임워크가 렌더한 컴포넌트까지.

지금은 경로(`pathname`)와 쿼리가 그대로인 history 이동은 무시합니다. 클릭 경로에는 원래부터
같은 규칙이 있었고(`shouldHandle`), 이제 history 이동에도 같습니다. 경로가 실제로 바뀌는
이동은 그대로 전환합니다.

증상이 남아 있다면 다음을 확인하십시오.

- 사용 중인 빌드가 이 수정을 포함하는지(`Kineto.version`).
- 컨테이너 밖에서 `history.pushState`로 경로를 바꾸고 있지는 않은지. 모듈은 경로가 바뀌면
  전환을 수행하므로, 라우터가 이미 화면을 그린 뒤라면 `data-kt-no-transition`으로 그 링크를
  제외하거나 모듈을 그 화면에서 쓰지 않는 편이 낫습니다.
- 회귀 검사는 `node tests/browser/page-transition-hash.mjs`입니다.
