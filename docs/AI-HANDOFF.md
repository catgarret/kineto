...
- **Option values are text**: write an option with `textContent`,
  `setAttribute` or a CSS property — never inside an HTML string. Only options
  NAMED for HTML (`html`, `template`, `hoverTemplate`, `uiTemplate`, Tooltip's
  `html: true`) may be parsed, and `SECURITY.md` lists them. Read a text option
  with `textOption()` (an empty attribute reads as `true`), a selector from an
  option with `selectAll()`, and escape an id inside a selector with
  `cssString()`. `tests/browser/markup-trust.mjs` holds this.
- **Collisions come from the contract**: an option named like another module
  (`hold`, `drag`, `progress`, `cursor`) is resolved by
  `src/activationOwners.js`, generated by `scripts/sync-playground-options.mjs`.
  Never hand-edit it; add the option to `kineto.features.json` and rebuild.
- **ScrollTrigger positions follow the page**: the core refreshes ScrollTrigger
  when the body height settles after a change (`src/layoutRefresh.js`). A change
  that keeps the height (a same-height tab swap) needs `Kineto.refresh()`.
