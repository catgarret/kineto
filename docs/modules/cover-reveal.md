...
여러 이미지가 있어도 각 요소에 `data-kt-color`, `data-kt-color-2`를 따로 지정하면 각 이미지가 자기 색 조합을 사용합니다. `mask:true`를 켜면 마지막 색상 패널을 만들지 않고 같은 자리를 실제 콘텐츠에만 적용되는 마스크가 대신합니다. 색상 패널의 부모는 마스킹하지 않으므로 패널이 사각형 조각처럼 잘리지 않습니다. 예를 들어 `layers:2`에서는 첫 번째 색상 패널 하나와 색 없는 콘텐츠 마스크 하나가 움직이며, `color2`는 그 재생에서 사용되지 않습니다.
