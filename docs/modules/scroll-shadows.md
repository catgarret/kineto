...
스크롤 컨테이너 가장자리에 그림자나 마스크 페이드를 표시해 남은 스크롤 방향을 알려줍니다. 시작과 끝에 닿으면 해당 방향의 효과가 자연스럽게 사라집니다. `mask`는 가장자리별 남은 거리 CSS 변수를 이용하는 그라데이션 페이드 방식이라, 콘텐츠 위에 그림자를 덧칠하지 않고 자연스럽게 사라집니다.
