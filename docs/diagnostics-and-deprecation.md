...
| 항목 | 상태 | 현재 약속 |
|---|---|---|
| public error code | 미착수 | 오류를 임의의 문자열 코드로 포장하지 않음 |
| opt-in debug output | 미착수 | 기본값은 조용한 성공·실패가 아니라 기존 callback/Promise 계약 유지 |
...
- `destroy()` 뒤에는 debug listener와 buffer가 남지 않아야 합니다.
...
현재 공개 API에는 이 절차를 시작할 항목이 없습니다. 다음 1.0 준비 작업은 실제 계약 변경이 생길 때 이 문서의 shape를 fixture·changelog·adapter QA와 함께 구체화하는 것입니다.
