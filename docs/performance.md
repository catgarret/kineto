...
않습니다. 제거에서도 전체 instance registry를 다시 훑지 않고 실제로 제거된
subtree 안의 source element만 정리합니다. 다른 위치로 reparent되어 flush 시점에
다시 연결된 요소는 유지합니다. 따라서 모듈 수나 한 번의 DOM commit 크기가
커져도 탐색 비용이 불필요하게 곱해지지 않습니다.
...
- 방문한 모든 프레임의 `Image` 객체를 teardown까지 보유하지 않습니다. 현재 preload 범위보다 넓은 내부 보존 창 밖의 로드 완료 프레임은 참조와 이벤트 핸들러를 해제하고, 역스크롤로 다시 필요해지면 재요청합니다. HTTP/decoded-resource 캐시 여부는 브라우저 정책에 따릅니다.
