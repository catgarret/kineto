...
| native scroll snap | 브라우저의 scroll container와 `scroll-snap-align`을 사용 | 아직 공개하지 않음. 단순 `slide` 전용 후보 |
...
다음 조건을 모두 만족하는 fixture가 확보되기 전에는 `scrollSnap` 공개 옵션을
추가하지 않습니다.

1. `effect:'slide'`, `loop:'off'`, `perView:1`, `axis:'x'`의 단순 구성에서
   transform 엔진과 같은 `index`, `isBeginning`, `isEnd`, change 이벤트를 보장합니다.
2. 마우스·touch·키보드·wheel 입력이 같은 슬라이드에 도착하고, 지원하지 않는
   브라우저에서는 기존 엔진으로 자동 fallback합니다.
...
`perView>1`, loop, 3D effect, radial, autoHeight는 첫 native fixture의 범위에서
제외합니다. 기능을 억지로 하나의 엔진에 섞으면 현재 해결한 Radial 고스트 이미지와
드래그 회귀를 다시 만들 가능성이 큽니다.
...
1. Node 회귀 테스트에서 momentum on/off, sticky snap, edge bounce의 target과
   `destroy()` 복원을 고정합니다.
2. Chromium에서 pointer velocity와 실제 layout rect를 측정합니다.
