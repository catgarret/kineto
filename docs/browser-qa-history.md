...
| 2026-09-19 | v0.10.0 stylized media release | `3228d26` | CI `35420646875` / Release `35420648097` | 성공 | 성공 | 성공 | `35421347873` **취소**(정적 `demo-site` concurrency 그룹을 Dependabot 브랜치의 skip된 Deploy 실행이 선점) → canonical은 `9b9449e` 유지, backup은 `3228d26` 서빙, parity 불일치 | Lazy dither·ascii·halftone 세 엔진 픽셀 검사, 설정창 gating 회귀, 새 Actions 메이저 통과. npm/GitHub tarball 563,063 bytes·SHA256 `50d6d0e0cbeb71ae8fa4b184a3fd2431ee792ff632b17014b2bb23b2584b658f`·provenance subject 일치. `pages.yml` 그룹을 실제 배포 전용 `demo-site-main`으로 수정(다음 main 푸시에서 재배포) |
| 2026-09-18 | v0.9.9 security/lifecycle batch | `ba0bb4b` | CI `35359625205` / Release `35359768823` | 성공 | 성공 | 성공 | `35361162158` 성공 (backup: 확인 시점 미동기화, parity 별도 기록) | 공유 링크 XSS 차단·SRI 태그 재사용·Presence subscribe·공용 스타일 스냅샷·배포 사이트 minify·Actions 메이저 고정을 검증. npm/GitHub tarball 548,364 bytes·SHA256 `f9104ab470361136d14fe9f01ff455ac97a717df456f3c985bd065779af210cd`·provenance subject 일치; 00:18 KST에 canonical의 build `ba0bb4b`·52개·GTM·자체 JS/CSS 12개·클릭 미디어 4개 일치 확인 |
