...
- Variants: `fade`, `blur-up`, `wave`, `grain`, `skeleton`, `pixelate`, `print`, `dissolve`, `flicker`, `polaroid`, `crt`, `data-mosaic`, `rgb-slice-burst`, `dither` (deprecated), `ascii` (deprecated), `halftone` (deprecated)
...
- Variants: `layout`, `none`, `slide`, `fade`, `crossfade`, `fade-slide`, `scale`, `fold`
- Public options: `duration`, `ease`, `foldBlur`, `item`, `mode`, `stagger`, `viewTransition`, `watch`
