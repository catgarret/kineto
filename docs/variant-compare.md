...
> Generated from `scripts/generate-variant-catalog.mjs` · library v0.12.2 · 38 modules / 228 variants.
...
| 방식 | 뜻 | 모듈 | variant |
|---|---|---|---|
| `grid` (나란히) | 같은 소재를 variant 수만큼 복제해 살아 있는 상태로 나란히 보여 줍니다. | 28개 | 185개 |
...
185/228 variant가 시트 안에서 그대로 살아 움직입니다. 나머지는 왜 그럴 수 없는지가 아래 표에 적혀 있습니다.
...
| `scrollVelocity` | `grid` | `data-kt-scroll-velocity="<variant>"` | 6 |  |
...
| `textTransition` | `grid` | `data-kt-text-transition="<variant>"` | 10 |  |
...
| `megaMenu` | `grid` | `data-kt-layout="<variant>"` | 3 |  |
| `gesture` | `grid` | `data-kt-gesture="<variant>"` | 2 |  |
