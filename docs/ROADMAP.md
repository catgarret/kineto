...
> 기준 버전: v0.12.2 · 작성일: 2026-08-02 · 검토: 2026-09-19
...
v0.9.9에서 공개 데모의 `?kt=` 공유 링크 XSS를 차단하고, SRI 검증 없는 엔진 태그
...
v0.11.0에서 “어떤 프레임워크·디자인 시스템·컴포넌트 라이브러리 옆에 붙여도 동작하고 어울리는 Kineto”를 목표로 연동 프로그램을 도입했습니다.
`Kineto.observe()`(29번째 Core API)가 나중에 렌더링되는 DOM을 자동으로 붙이고 정리하며, 하나의 연동 지도
...
Kineto MCP 서버(`packages/kineto-mcp`, 독립 태그 `mcp-v*`로 릴리스) 계약 사본을 생성합니다. 주장은 실제
라이브러리로 `tests/integrations/`에서, Bootstrap 5 공존 예제는 실제 브라우저에서 검증합니다(§3 원칙 참조).
Pages 배포 concurrency 그룹을 실제 배포 전용으로 좁혀 v0.10.0에서 취소됐던 canonical 배포 문제도 함께
...
v0.12.1에서 공개 모듈이 52개에서 55개가 되었습니다(`v0.12.0` 태그는 릴리스 검사가 실패해 게시되지 않았고, 원인과 수정은
CHANGELOG 0.12.1 항목에 있습니다. `v0.12.1` 태그도 게시되지 않아 이 내용은 v0.12.2로 처음 배포됩니다). Lazy에서 분리한 Stylize(디더·아스키·하프톤을 영구 필터로,
...
같은 저장소를 동시에 다룬 첫 릴리스이기도 해서, 작업 브랜치·PR 우선 규칙(`AGENTS.md`)을 함께 넣었습니다. 릴리스 직전
소유자 데모 리뷰에서 나온 문제는 증상이 아니라 원인 계열로 닫았습니다 — 손으로 적은 목록의 어긋남(옵션 이름 충돌을
...
v0.12.2에서 0.12 라인이 처음 npm에 게시됩니다. 내용은 v0.12.1과 같고, 검증 파이프라인을 다시 짰습니다 —
`v0.12.1`은 CI가 통과한 커밋을 릴리스가 같은 검사로 다시 돌리다 흔들리는 테스트 하나에 실패해 게시되지
않았기 때문입니다. 테스트 목록은 `package.json`에만 두고 CI가 이를 병렬 shard로 나눠 돌리며(약 25분 → 6~10분),
릴리스는 태그 커밋의 CI 성공을 확인한 뒤 패키지만 검사합니다(약 25분 → 5분). 재시도에서야 통과한 테스트는
flaky로 보고되고, 모든 push 전에 `npm run verify:push`를 돌립니다. 데모는 세 개짜리 줄 뒤에서 카드 하나로
끝나지 않도록 마지막 두 줄을 나눕니다. 원인 분석은 [아키텍처 리뷰](ARCHITECTURE-REVIEW.md) §2.6~2.7에 있습니다.
휴대폰 성능도 여기서 잡았습니다: `Kineto.config({ defer: true })`로 시각 효과를 화면 근처에서 만들고,
...
- 55개 모듈을 `data-kt-*`, JavaScript API, `core`와 `modules/*` 엔트리로 제공합니다.
...
- Chromium 전체 QA와 Chromium·Firefox·WebKit 53/53 공개 모듈 lifecycle smoke가 있으며, transform·clip·fixed/sticky 의존 모듈은 세 엔진의 `heavy-layout` 체크포인트로 별도 검증합니다. lifecycle·패키지·번들 예산·npm provenance 검증도 함께 유지합니다.
...
- 55개 모듈의 사용 시점·피해야 할 상황·접근성·성능·reduced motion 상태를 단일 생성 원본과 데모 뱃지, 문서 매트릭스, CI completeness 검사로 연결했습니다.
...
- 55개 모듈과 7개 언어는 유지 비용이 큽니다. 새 기능이 기존 기능의 품질과 문서화를 밀어낼 위험이 있습니다.
...
- **variant 구분성은 정적 게이트와 실제 시각 검증을 분리해 관리합니다.** `pageReveal` 16개와 Reveal·Lazy·Stylize·Cursor·Overflow Text·Glitch·Slider·Text Transition 94개는 contract·구현 branch/source fingerprint·생성 설정의 1:1 대응을 자동 검사합니다. 확대 범위 중 전용 데모 markup은 82/91이며(2026-09-24에 감사에 들어온 Text Transition은 10개 중 `blur`·`pop`만 전용 카드 — 나머지는 설정과 비교 시트에서 재생)(지원 중단 예정 3개는 대상에서 제외 — 선택지처럼 보이는 전용 카드를 주지 않고 [variant 비교 시트](variant-compare.md)가 배지와 함께 보여 줍니다) Slider 효과는 9개가 전용 카드이고 `radial`은 같은 엔진을 쓰는 Radial 블록의 카드로 비교합니다. source fingerprint는 픽셀 단위 시각 회귀를 대신하지 않습니다. 남은 병목은 이름 수가 아니라 실제 회귀 신호가 있는 variant의 전용 비교 화면과 cross-browser 검증을 비용에 맞춰 확대하는 일입니다.
...
- 53개 reference와 별도로 문제 중심 가이드를 만듭니다.
