...
- **variant 구분성은 정적 게이트와 실제 시각 검증을 분리해 관리합니다.** `pageReveal` 16개와 Reveal·Lazy·Cursor·Overflow Text·Glitch·Slider 78개는 contract·구현 branch/source fingerprint·생성 설정의 1:1 대응을 자동 검사합니다. 확대 범위 중 전용 데모 markup은 67/78이며 Slider 10개 효과는 전용 카드로 비교합니다. source fingerprint는 픽셀 단위 시각 회귀를 대신하지 않습니다. 남은 병목은 이름 수가 아니라 실제 회귀 신호가 있는 variant의 전용 비교 화면과 cross-browser 검증을 비용에 맞춰 확대하는 일입니다.
