...
`ci`는 lint, ESM/UMD build, 55개 모듈·31개 Core API, 52개 요구사항, 생성 문서, Chromium 기능/lifecycle, animated media, 플레이그라운드, package surface와 tarball 내용을 확인합니다. 배포 전 `npm run verify`를 실행하면 같은 전체 검사에 의존성 보안 감사까지 추가됩니다.
