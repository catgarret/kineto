...
(function () {
  ...
  const stylizeOnly = {
      "ko": {
          ...
          "holdDuration": "원본이 드러난 뒤 레이어 제거와 완료 콜백까지 기다리는 시간(ms)입니다. 영상에서는 일시정지·숨김 탭 시간을 제외합니다.",
      ...
      "en": {
          ...
          "holdDuration": "Wait after revealing the original before layer removal and the completion callback (ms). Videos exclude paused and hidden-tab time.",
      ...
      "ja": {
          ...
          "holdDuration": "原本が現れた後、レイヤー削除と完了コールバックまで待つ時間(ms)。動画では一時停止・非表示タブの時間を除きます。",
      ...
      "zh-CN": {
          ...
          "holdDuration": "显示原始画面后，到移除图层并调用完成回调的等待时间（毫秒）。视频不计入暂停和标签页隐藏的时间。",
      ...
      "zh-TW": {
          ...
          "holdDuration": "顯示原始畫面後，到移除圖層並呼叫完成回呼的等待時間（毫秒）。影片不計入暫停及分頁隱藏的時間。",
      ...
      "ru": {
          ...
          "holdDuration": "Ожидание после показа оригинала до удаления слоя и вызова обработчика завершения (мс). Для видео время паузы и скрытой вкладки не учитывается.",
      ...
      "it": {
          ...
          "holdDuration": "Attesa dopo la comparsa dell'originale, prima di rimuovere il livello e chiamare il callback finale (ms). Per i video esclude pause e schede nascoste.",
