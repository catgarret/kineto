...
// Panel IDs must be unique across simultaneous Mega Menu instances. Generated
// IDs belong to Kineto and are removed on destroy; authored IDs and trigger
// ARIA present before initialization must be restored exactly.
const generatedMenuHostA = document.createElement('nav');
generatedMenuHostA.innerHTML = `
  <ul><li>
    <button aria-controls="legacy-panel" aria-haspopup="menu" aria-expanded="true">Products</button>
    <div class="kt-menu-panel"><a href="#">Product</a></div>
  </li></ul>`;
const generatedMenuHostB = document.createElement('nav');
generatedMenuHostB.innerHTML = `
  <ul>
    <li><button>Solutions</button><div class="kt-menu-panel"><a href="#">Solution</a></div></li>
    <li><button>Support</button><div id="authored-support-panel" class="kt-menu-panel"><a href="#">Support</a></div></li>
  </ul>`;
document.body.append(generatedMenuHostA, generatedMenuHostB);

const generatedMenuA = megaMenuModule.create(generatedMenuHostA);
const generatedMenuB = megaMenuModule.create(generatedMenuHostB);
const generatedPanelA = generatedMenuHostA.querySelector('.kt-menu-panel');
const generatedPanelB = generatedMenuHostB.querySelector('.kt-menu-panel');
const authoredPanel = generatedMenuHostB.querySelector('#authored-support-panel');
const generatedTriggerA = generatedMenuHostA.querySelector('button');
const [generatedTriggerB, authoredTrigger] = generatedMenuHostB.querySelectorAll('button');
const firstGeneratedId = generatedPanelA.id;

assert.ok(firstGeneratedId, 'Mega Menu must assign an ID when a panel has none');
assert.ok(generatedPanelB.id, 'every generated Mega Menu panel must receive an ID');
assert.notEqual(generatedPanelB.id, firstGeneratedId, 'simultaneous Mega Menu instances must not duplicate generated panel IDs');
assert.equal(generatedTriggerA.getAttribute('aria-controls'), firstGeneratedId, 'generated trigger controls must reference its own panel');
assert.equal(generatedTriggerB.getAttribute('aria-controls'), generatedPanelB.id, 'second generated trigger controls must reference its own panel');
assert.equal(authoredTrigger.getAttribute('aria-controls'), authoredPanel.id, 'authored panel IDs must remain the trigger target');

generatedMenuA.destroy();
assert.equal(generatedPanelA.hasAttribute('id'), false, 'destroy must remove an ID generated by Mega Menu');
assert.equal(generatedTriggerA.getAttribute('aria-controls'), 'legacy-panel', 'destroy must restore the original aria-controls value');
assert.equal(generatedTriggerA.getAttribute('aria-haspopup'), 'menu', 'destroy must restore the original aria-haspopup value');
assert.equal(generatedTriggerA.getAttribute('aria-expanded'), 'true', 'destroy must restore the original aria-expanded value');

const recreatedMenuA = megaMenuModule.create(generatedMenuHostA);
assert.ok(generatedPanelA.id, 'recreating a destroyed Mega Menu must assign a panel ID again');
assert.notEqual(generatedPanelA.id, firstGeneratedId, 'recreated Mega Menu panels must not reuse an earlier generated ID');
assert.notEqual(generatedPanelA.id, generatedPanelB.id, 'recreated and still-active Mega Menu panel IDs must stay unique');
assert.equal(generatedTriggerA.getAttribute('aria-controls'), generatedPanelA.id, 'recreated trigger controls must follow the new generated ID');
recreatedMenuA.destroy();
generatedMenuB.destroy();

assert.equal(generatedPanelA.hasAttribute('id'), false, 'recreated generated IDs must also clean up on destroy');
assert.equal(generatedPanelB.hasAttribute('id'), false, 'destroy must clean up generated IDs from every instance');
assert.equal(authoredPanel.id, 'authored-support-panel', 'destroy must preserve an author-supplied panel ID');
assert.equal(generatedTriggerB.hasAttribute('aria-controls'), false, 'destroy must remove generated trigger ARIA when none existed before');
assert.equal(authoredTrigger.hasAttribute('aria-controls'), false, 'destroy must remove authored-panel trigger ARIA when none existed before');
generatedMenuHostA.remove();
generatedMenuHostB.remove();

// Destroy must restore the complete author-owned menu state, including a
// pre-existing hidden panel, authored module-like classes, inline panel styles,
// and a panel that began visible. It must also cancel a closing/opening Web
// Animation without letting its oncancel callback overwrite the snapshot.
const lifecycleMenuHost = document.createElement('nav');
lifecycleMenuHost.className = 'author-nav kt-menu';
lifecycleMenuHost.innerHTML = `
  <ul>
    <li class="author-item kt-open">
      <button class="author-trigger kt-menu-trigger" aria-expanded="mixed">Hidden initially</button>
      <div id="author-hidden-panel" class="kt-menu-panel author-panel" style="color: navy" hidden><a href="#">Hidden item</a></div>
    </li>
    <li class="author-visible-item">
      <button class="author-visible-trigger">Visible initially</button>
      <div id="author-visible-panel" class="kt-menu-panel author-visible-panel" style="--kt-menu-panel-top: 47px; color: maroon"><a href="#">Visible item</a></div>
    </li>
  </ul>`;
document.body.appendChild(lifecycleMenuHost);
const lifecycleItems = [...lifecycleMenuHost.querySelectorAll('li')];
const lifecycleTriggers = [...lifecycleMenuHost.querySelectorAll('button')];
const lifecyclePanels = [...lifecycleMenuHost.querySelectorAll('.kt-menu-panel')];
const lifecycleNodes = [lifecycleMenuHost, ...lifecycleItems, ...lifecycleTriggers, ...lifecyclePanels];
const nodeAttributes = (node) => Object.fromEntries(
  [...node.attributes].map(({ name, value }) => [name, value]).sort(([a], [b]) => a.localeCompare(b))
);
const authoredMenuState = lifecycleNodes.map(nodeAttributes);
const menuPlayers = [];
lifecyclePanels.forEach((panel) => {
  panel.animate = () => {
    const player = {
      cancelled: false,
      onfinish: null,
...
const authoredInnerWidth = window.innerWidth;
Object.defineProperty(window, 'innerWidth', { configurable: true, value: 390 });

const lifecycleMenu = megaMenuModule.create(lifecycleMenuHost, {
  trigger: 'click', responsive: 'scroll', duration: 1
});
assert.equal(lifecyclePanels[0].hidden, true, 'initially hidden authored panel must stay hidden while closed');
assert.equal(lifecyclePanels[1].hidden, true, 'initially visible authored panel must be closed during enhancement');
lifecycleTriggers[1].dispatchEvent(new window.MouseEvent('click', { bubbles: true, cancelable: true, detail: 1 }));
assert.equal(lifecyclePanels[1].hidden, false, 'click behavior must still open the lifecycle fixture');
assert.equal(lifecyclePanels[1].style.getPropertyValue('--kt-menu-panel-top'), '12px', 'mobile placement must still update the panel inline variable');
lifecycleTriggers[1].dispatchEvent(new window.KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true }));
assert.equal(menuPlayers.length, 2, 'open followed by Escape must create open and close animations');
assert.equal(menuPlayers[0].cancelled, true, 'starting the close must cancel the in-progress open animation');
assert.equal(menuPlayers[1].cancelled, false, 'the close animation must still be running before destroy');
lifecycleMenu.destroy();
assert.ok(menuPlayers.every(({ cancelled }) => cancelled), 'destroy must cancel every active Mega Menu animation');
assert.equal(menuPlayers[1].oncancel, null, 'destroy must detach the close callback before animation cancellation');
assert.deepEqual(lifecycleNodes.map(nodeAttributes), authoredMenuState, 'destroy must restore every author-owned class, style, hidden, ID, and ARIA attribute');
assert.equal(lifecyclePanels[0].hidden, true, 'destroy must restore authored hidden=true');
assert.equal(lifecyclePanels[1].hidden, false, 'destroy must restore authored hidden=false');

const recreatedLifecycleMenu = megaMenuModule.create(lifecycleMenuHost, {
  trigger: 'click', responsive: 'scroll', duration: 1
});
lifecycleTriggers[1].dispatchEvent(new window.MouseEvent('click', { bubbles: true, cancelable: true, detail: 1 }));
const recreatedPlayer = menuPlayers.at(-1);
recreatedLifecycleMenu.destroy();
assert.equal(recreatedPlayer.cancelled, true, 'recreated Mega Menu destroy must cancel its running animation');
assert.deepEqual(lifecycleNodes.map(nodeAttributes), authoredMenuState, 'destroy after recreate must restore the same authored snapshot');
Object.defineProperty(window, 'innerWidth', { configurable: true, value: authoredInnerWidth });
lifecycleMenuHost.remove();
