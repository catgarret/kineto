...
// Integration program: every entry point reaches the generated guides, the AI
// rules, the shadcn registry and the MCP server.
assert.match(readme, /\[Integrations\]\(docs\/integrations\/README\.md\)/, 'root README must link the integration guides');
for (const token of ['Kineto.observe()', 'kineto.dongri.me/r/registry.json', 'ai/kineto.rules.md', 'packages/kineto-mcp/README.md', 'examples/bootstrap/index.html', 'kineto.integrations.json']) {
  assert.ok(readme.includes(token), `root README must mention ${token}`);
}
assert.match(docsReadme, /\[연동 가이드 인덱스\]\(integrations\/README\.md\)/, 'docs index must link the integration guides');
assert.match(docsReadme, /\[Kineto MCP 서버\]\(\.\.\/packages\/kineto-mcp\/README\.md\)/, 'docs index must link the MCP server');
assert.ok(localizedReadmes.every((content) => content.includes('../docs/integrations/README.md') && content.includes('Kineto.observe()')), 'every localized README must link the integration guides and mention observe()');
assert.match(read('docs/getting-started.md'), /Kineto\.observe\(\)/, 'getting started must introduce observe()');
assert.match(read('AI-PROMPT-GUIDE.md'), /kineto_suggest/, 'the AI prompt guide must point agents at the MCP tools');
for (const file of fs.readdirSync(path.join(root, 'docs/integrations'))) {
  assert.match(read(`docs/integrations/${file}`), /Generated by scripts\/generate-integrations\.mjs/, `${file} must be generated, not hand-edited`);
}
