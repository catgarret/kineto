...
// Option names that are ALSO another module's activation name. On a slider,
// `data-kt-progress` means "show autoplay progress", and it must not also start
// the Progress module on the same element. The core and the playground both
// need this map; both used to type it by hand, and both were missing entries
// (Text Transition's and Brush Reveal's `hold`, Slider's `drag`, Loader's
// `progress`) — so `data-kt-hold="1400"` on a Text Transition also turned the
// element into a Hold button. It is derived here, from the contract, instead.
const names = new Set(feat.modules.map((m) => m.name));
const owners = {};
for (const m of [...feat.modules].sort((a, b) => a.name.localeCompare(b.name))) {
  for (const option of [...m.publicOptions].sort()) {
    if (option !== m.name && names.has(option)) (owners[option] = owners[option] || []).push(m.name);
  }
}
const ownersSorted = Object.fromEntries(Object.keys(owners).sort().map((key) => [key, owners[key]]));
const ownersLiteral = `  const ACTIVATION_OPTION_OWNERS = ${JSON.stringify(ownersSorted)};`;
const ownersModule = [
  ...
  `export const ACTIVATION_OPTION_OWNERS = Object.freeze(${JSON.stringify(ownersSorted, null, 2)});`,
  ''
].join('\n');
const ownersFile = path.join(root, 'src/activationOwners.js');
...
const ownersRe = /^ {2}const ACTIVATION_OPTION_OWNERS = \{.*\};$/m;
if (!ownersRe.test(src)) { console.error('sync-playground-options: ACTIVATION_OPTION_OWNERS block not found'); process.exit(1); }
const next = src.replace(re, literal).replace(variantsRe, variantsLiteral).replace(reqRe, requiresLiteral).replace(defRe, defaultsLiteral).replace(varOptRe, variantOptionsLiteral).replace(attrRe, attributesLiteral).replace(ownersRe, ownersLiteral);
const ownersCurrent = fs.existsSync(ownersFile) ? fs.readFileSync(ownersFile, 'utf8') : '';

if (check) {
  if (ownersCurrent !== ownersModule) { console.error('src/activationOwners.js is out of sync with kineto.features.json. Run: node scripts/sync-playground-options.mjs'); process.exit(1); }
  ...
  if (ownersCurrent !== ownersModule) { fs.writeFileSync(ownersFile, ownersModule); console.log('Synced src/activationOwners.js from features.json.'); }
