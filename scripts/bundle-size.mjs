...
const BUDGETS = {
  ...
  // Native Reveal boundaries/reverse: 561.2 / 440.2 / 438.3 KiB raw;
  // compressed ceilings, runner variance and consumer budgets stay unchanged.
  // 2026-09-20 (control labels): default label maps in slider, lightbox,
  // fullpage and toast plus the shared `labeller` helper measure 562.3 KiB raw
  // ESM and 439.1 KiB raw UMD. The strings themselves already shipped as inline
  ...
  // 2026-09-21 (squircle, module 54): the superellipse geometry and the
  // corner-shape polyfill measure 574.3 KiB raw / 154.8 KiB gzip ESM,
  ...
  // 2026-09-21 (radial menu): Mega-menu's radial layout, the shared
  // numberOption reader and the ring's CSS measure 590.1 / 159.3 ESM, 462.4 /
  // 141.7 minified and 460.5 / 141.1 UMD, with the stylesheet at 45.0 KiB raw.
  ...
  // 2026-09-24 (text-transition pop): the sampled spring, the per-instance
  // effect builders that replace the shared, mutated keyframe table, and the
  // quick-leave path measure 612.0 / 166.6 ESM, 479.3 / 148.1 minified and
  // 477.3 / 147.5 UMD. Same rounding rule as above.
  // 2026-09-24 (integrated with the parallel perf branches: tiered scan
  // discovery, progress-output write dedupe, bounded Scroll Sequence cache):
  // 613.4 / 167.0 ESM, 480.1 / 148.5 minified, 478.2 / 147.8 UMD. Same rule;
  // the UMD raw ceiling moves by two because deps-boundary reads it strictly.
  'kineto.js': { raw: 614, gz: 168, variance: 2 },
  ...
  'kineto.min.js': { raw: 481, gz: 149, variance: 2 },
  ...
  // strict upper bound on the shipped file.
  'kineto.umd.js': { raw: 480, gz: 148, variance: 1 },
  'kineto.umd.min.js': { raw: 480, gz: 148, variance: 1 },
